...
import com.pro.common.multitenant.CurrentTenantIdentifierResolverImpl;
import com.pro.common.multitenant.MapMultiTenantConnectionProvider;
 ...
 */
@Configuration
@EnableJpaRepositories(basePackages = { "com.pro.common" }, transactionManagerRef = "txManager")
...
//		
		...
		dataSourceDefault.setIdleConnectionTestPeriod(Integer.parseInt(CONN_POOL_IDLE_PERIOD));
		
		dataSourceDefault.setAcquireIncrement(Integer.parseInt(ACQUIRE_INCREMENT));
		dataSourceDefault.setAcquireRetryAttempts(Integer.parseInt(ACQUIRE_RETRY_ATTEMPTS));
		dataSourceDefault.setAcquireRetryDelay(Integer.parseInt(ACQUIRE_RETRY_DELAY));
		
		dataSourceDefault.setCheckoutTimeout(Integer.parseInt(TIME_OUT));
		dataSourceDefault.setMaxStatements(Integer.parseInt(MAX_STATEMENT_PER_CONNECTION));
		dataSourceDefault.setNumHelperThreads(Integer.parseInt(NO_OF_HELPER_THREADS));
		dataSourceDefault.setAutoCommitOnClose(true);
		dataSourceDefault.setUnreturnedConnectionTimeout(Integer.parseInt(TIME_OUT));
